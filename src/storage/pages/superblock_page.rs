...
impl<'buf> SuperblockPage<&'buf mut [u8; PAGE_SIZE]> {
    ...
    pub(crate) fn new_with_buffer(
        buffer: &'buf mut [u8; PAGE_SIZE], page_size: u16, page_id: u64, tx_id: u64,
        bump_alloc_page_id_start: u64,
    ) -> Self {
    ...
    pub(crate) fn new_with_buffer_from_header(
        buffer: &'buf mut [u8; PAGE_SIZE], header: &SuperblockHeader,
    ) -> Self {
